...
| `PROV-005` | PROV | Provider request key、成功复用、调用上限、有限重试、耗时/用量/成本统计均版本化。Visual primary escalation groups 限 `4/page`、`8/project`，与 text+visual actual-attempt 统一上限 `16/page` 及 wall budget `45s/page`、`90s/project` 分开记账；retry 不增加 primary count，但每次实际 attempt 都消耗 unified count 和 wall time。Cache hit 只在 content identity 与 model/prompt/schema/adapter/proposal/router/PyMuPDF provenance 全兼容时成立。Paid verification cycle 必须绑定 one-use authorization、literal run/project admission 与版本化公开 list-price snapshot；每次真实 network submission 都在调用前重新验证 active admission并持久化保守 reservation，只有安全完整 usage 才可向下结算，missing/invalid usage、Provider failure 与中断保留 reservation，cycle-wide committed cost 超 ceiling 时禁止调用。Cache hit、Provider factory failure 与 admitted-but-never-submitted group 不产生 paid entry；UI 编辑和重新导出不得触发无关调用。 | Provider-call policy Owner | Processing, cache, telemetry, Harness policy | request/content key inputs; provenance identity; cycle/run/project authorization; pricing snapshot identity; reuse state; primary/actual/retry/wall budgets; cycle-wide reservation/settlement ledger; usage/cost summary | raw token detail、account invoice、discount/free-tier、backoff trace、budget projection | `CR-ID`, `CR-EVAL`, `CR-ADD` | `BR-ID`, `BR-EVAL`, `BR-STAGE` | `v0.1-design` | `P0-partial` |
