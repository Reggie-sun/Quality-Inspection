...
| `REV-002` | REV | Raw automatic result 冻结自动能力证据且不可变，包括 technical requirement 的重建、分类、匹配 decision 和 provenance；人工编辑不使其失效，也不能用 reviewed result 冒充自动能力。 | Automatic-result Owner | Review, evaluation, diagnostics | raw result ID; inputs; candidates; coverage; technical requirement decisions; Provider refs; created-at | evaluation cache、debug annotations | `CR-IMM`, `CR-EVAL` | `BR-IMM`, `BR-EVAL` | `v0.1-design` | `P0` |
