...
| `CAND-004` | CAND | Vision LLM 只按需复核局部 candidate 或 suspicious visual-observation crop 并输出受 schema 约束的建议；确定性 validator 决定是否接受建议和 disposition，模型不得猜测不足上下文或成为正式 Owner；任何 Provider confidence 只作为 signal，不能提交 auto acceptance。 | Candidate Advisor boundary | Provider adapter, parser, coverage, review | review reason; crop/source refs; prompt/schema/model versions; validated suggestion; confidence signal | raw response、expanded crop、token trace | `CR-OWNER`, `CR-ADD` | `BR-OWNER`, `BR-SCHEMA` | `v0.1-design` | `P0` |
| `CAND-005` | CAND | Coverage Ledger 覆盖所有疑似工程 text/visual observation IDs；缺 disposition/source/coordinates、冲突归属、未调度 visual observation 或 visual budget overflow 均为 blocking，不允许静默丢弃。 | Coverage Owner and Veto Gate | Processing, review, acceptance | entry/observation identity; disposition; source; coordinates; scheduling state; severity; checked status | ledger explanation、coverage heatmap | `CR-ID`, `CR-OWNER` | `BR-ID`, `BR-OWNER`, `BR-PUBLISH` | `v0.1-design` | `P0` |
