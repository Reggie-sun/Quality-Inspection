...
- consume使用 `O_CREAT|O_EXCL|O_NOFOLLOW` 创建单一 consumption document并 `fsync` file/directory。重复或并发 consume均 fail closed；
...
- accepted pause后，`execute-resume` 必须在任何credential/runtime activation之前用 `O_CREAT|O_EXCL|O_NOFOLLOW` 创建并fsync唯一 `resume-consumed` fact，绑定authorization identity、literal run ID、accepted-pause evidence hash与safe timestamp。重复、并发、不同run/evidence或terminal后的resume全部fail closed；fact创建后的任何contracts/preflight/run/cleanup failure都消费这次resume，禁止再次resume；
...
Start/resume都由单一Python lifecycle orchestrator拥有，不依赖Make shell链或后续人工cleanup。`execute-start` 在cycle consume前安装 `SIGINT`/`SIGTERM` handler并进入 `try/finally`；`execute-resume` 在exclusive consume `resume-consumed` fact前安装同等handler，只接受literal paused run，并在fact fsync后才允许重新激活。Pre-consume validation rejection执行zero mutation/activation；resume fact一经创建则不可重用，之后任意exit都必须close并deactivate。Activation、contracts、preflight、run failure或signal时，orchestrator先stop/quiesce worker、通过唯一close bridge写failed/aborted terminal，再用idempotent `deactivate-runtime`移除四credentials/cycle/auth mount；accepted pause时不close cycle，但仍在返回前立即deactivate credentials/runtime，Task 9 literal resume时才重新激活。Cleanup helper failure必须保留nonzero exit并写durable blocker，不能被primary status吞掉。
...
- authorization issued但尚未consumed；state dir/backup ownership/mode/hash正确；
...
Harness先open/bind唯一run，再将project create/upload与processing拆成两个受控阶段。project DB row创建后立即持久化run-bound project ID和host-created admission，不能等 `AutomaticResult`，只有admission durable后才触发processing。不论success/failure，都收集并seal：authorization、pricing snapshot、cycle-wide usage ledger、routing decision/attempt/outcome aggregate、storage artifact inventory、container monitor与command exit。
...
- budget reservation拒绝的group是never-submitted budget terminal，zero paid artifact，并阻止Step 4 success。
