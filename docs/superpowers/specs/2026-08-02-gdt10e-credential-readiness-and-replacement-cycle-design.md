...
- 新增 `.agent/harness/scripts/provider_account_readiness.py` 只拥有 private operator attestation schema、expiry与 credential-bundle binding；它不是 Provider、budget或cycle Owner。
- `live_cycle_authorization.py`继续是 issuance/consume/run/project/resume/terminal lifecycle Owner，只消费 readiness document SHA 与 validated binding verdict。
...
`provider_account_readiness.py`提供且只提供以下CLI：`issue`、`validate --phase start|resume`和`dispose`。`issue`从process environment读取`QI_QWEN_API_KEY`、`QI_QWEN_WORKSPACE_ID`、`QI_QWEN_MODEL`和`QI_P0_OPERATOR_ID`，禁止在argv接收key/workspace；`validate`只输出sanitized JSON；`dispose`要求exact content SHA、exact allowlisted root和absence/terminal lifecycle proof后fsync-delete。任何unexpected child、owner/mode/symlink/hash mismatch均停止且不删除。
...
Zero-paid `NO-GO`必须调用lifecycle Owner的`abort-preconsume`，而不是人工删除。无issuance分支验证runtime未activation、authorization root不存在；issued-but-unconsumed分支先exclusive-create/fsync `provider-cycle-unconsumed-cancellation/1`，要求issuance存在且consumption/run/project/resume/terminal/activation全部不存在。Before deletion，Owner validates all bytes and O_EXCL/fsyncs exact sibling `/var/tmp/quality-inspection-gdt10e-20260802-db2265ae5e7d-cleanup-intent.json`。唯一删除顺序是`live/safe override -> preparation/zero-paid reports -> account-readiness.json -> authorization files/root -> empty private root`；后续步骤只读取intent中预先冻结的safe hashes，不依赖已删readiness。Success O_EXCL/fsyncs `/var/tmp/quality-inspection-gdt10e-20260802-db2265ae5e7d-cleanup-receipt.json`后删除intent；failure/interruption保留intent并O_EXCL/fsyncs `/var/tmp/quality-inspection-gdt10e-20260802-db2265ae5e7d-cleanup-blocker.json`。Receipt/blocker互斥，existing file只接受exact schema/hash replay。Intent先于任何删除持久化，因此partial deletion即使blocker写入中断也有durable owner/deadline state。Blocker记录execution owner、uid/gid、mode、safe path SHA、expiry、completed-step booleans和review deadline `2026-08-09T23:59:59+08:00`，绝不记录raw path/secret；blocker必须在parent plan登记且禁止issue/consume/start。
...
Design/plan acceptance只要求：read-only audit完整、方案/预算/Owner/retry/stop/evidence边界无歧义、独立review `accept`。它不等于execution approval。
