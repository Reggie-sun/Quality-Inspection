...
For `terminal` only, the map additionally and exactly has six run-bound keys, derived from validated literal `${run_id}` under `/home/reggie/vscode_folder/Quality_Inspection/.worktrees/structured-geometric-tolerance-recognition/.agent/harness/runs`: `harness_run_root`=`.../runs/${run_id}`; `harness_run_document`=`.../runs/${run_id}/run.json`; `harness_live_evidence`=`.../runs/${run_id}/live-run-evidence.json`; `harness_runtime_acceptance`=`.../runs/${run_id}/reports/provider-account-runtime-acceptance.json`; `harness_quiescence`=`.../runs/${run_id}/reports/provider-cycle-quiescence.json`; and `harness_close_bridge`=`.../runs/${run_id}/reports/provider-cycle-close-bridge.json`。Non-terminal maps reject these six keys; terminal maps require all six.
