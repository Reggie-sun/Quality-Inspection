...
- Runtime contract prerequisite：现有GDT-10D-era `0014` enforcement不能直接用于successor GDT-10E。DB mutation前必须按companion Step 1A以TDD只修改其列出的六个code/test paths：exact GDT-10E cycle唯一写入/接受`0016`，generic/historical GDT-10D继续唯一接受`0014`；禁止caller-supplied arbitrary revision、broad allowlist或fallback。Parent full focused verification、fresh independent implementation review、exact-file commit和HEAD runtime-closure check全部通过后，Step 1B reconciliation才可开始。
