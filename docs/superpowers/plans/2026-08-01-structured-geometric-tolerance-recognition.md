...
- Date: `2026-08-01`
- Status: `GDT-10 Step 4 authorized; paid invocation not consumed; isolated live-target binding amendment accepted and ready to commit`
- Execution order: `GDT-1 -> GDT-2 -> GDT-3 -> GDT-4 -> GDT-5 -> GDT-6 -> GDT-7 -> GDT-8 -> GDT-9 -> GDT-10`
- Current blocker: paid invocation has not started。Feature-only Compose isolation is active and main remains untouched，but Harness still hardcodes the superseded main HTTP target and does not bind selected Compose project to published API/frontend ports。This must fail closed and be replaced before credential injection、registration、upload or Provider work；standing authorization does not permit bypassing the stale target contract。
