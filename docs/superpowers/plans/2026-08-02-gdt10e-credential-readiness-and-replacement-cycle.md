...
## Latest-Main Paid Target Convergence Amendment — 2026-08-03

- Supersession boundary：本 amendment 仅替换本文件 Successor Step 0 selection record 的 no-merge/no-absorb 约束和 Deferred integration boundary 的本轮 no-paid 约束；Step 0 archive 保持已完成且禁止重跑。其它 budget、cycle、DB、review、zero-paid、cleanup与promotion veto全部继续生效。
- User decision：用户明确选择“整合最新 main”，要求本次 paid current-four 验证包含 `main@fe22698`，特别是 `501fd42` 的 `automatic-result/3 -> ReviewWorkingCopy` confidence projection 修复和 `fe22698` 的三层 auto-accepted Harness invariant；不得付费验证不含这两个 commit 的旧 feature worktree。
- Selected execution worktree：`.worktrees/gdt10e-retry-archive-continuation`。先关闭并提交当前 Step 1A 七文件改动，再把 `main@fe22698` merge 到 successor；不得 rebase、丢弃或覆盖现有 Step 1A work。merge conflict 只允许按已批准 Owner 保留双方语义，并在完整 offline gate 与独立 review 后提交。
- Runtime/run-tree convergence：Compose project、ports、volumes、cycle ID、budget、Provider/model和DB `0016` target保持不变；formal Harness run、runtime-acceptance、terminal cleanup和same-run resume的唯一 filesystem root改为 successor worktree的 `.agent/harness/runs`。旧 `.worktrees/structured-geometric-tolerance-recognition/.agent/harness/runs` 只保留 immutable historical evidence，不再接收新的 GDT-10E run。
- Single path Owner：`.agent/harness/scripts/provider_account_readiness.py::HARNESS_RUNS_ROOT` 继续唯一拥有 private resume/terminal run-root allowlist；`live_cycle_authorization.py::_terminal_cleanup_paths()` 必须从该 module读取同一 Path，不再从自己的 `ROOT`独立派生。不得增加 environment/CLI fallback。必须同步 design、plan、exact path tests和privacy/cleanup replay tests。
...
2. `runtime-acceptance` must resolve without symlink under `/home/reggie/vscode_folder/Quality_Inspection/.worktrees/gdt10e-retry-archive-continuation/.agent/harness/runs/` to the exact child `reports/provider-account-runtime-acceptance.json`；its run-directory segment must match `^[0-9]{8}T[0-9]{12}Z-[0-9a-f]{8}$` and equal the validated authorization binding；
...
Implement the exact safe-path map from the design: every branch has exactly the 18 named private-root, readiness, live/safe override, authorization root/children (including legacy child blocker), reports and root-sibling journal paths; only `terminal` additionally has exactly the six literal-run Harness root/document/live-evidence/runtime-acceptance/quiescence/close-bridge paths under `/home/reggie/vscode_folder/Quality_Inspection/.worktrees/gdt10e-retry-archive-continuation/.agent/harness/runs/${run_id}`. `live_cycle_authorization.py::_terminal_cleanup_paths()` must obtain this root from `provider_account_readiness.py::HARNESS_RUNS_ROOT`; it may not independently rebuild it from `ROOT`. Non-terminal mappings reject those six keys; terminal requires them. `expected_steps` is exactly `["safe_runtime_proved","live_override_absent","safe_override_absent","preparation_report_absent","zero_paid_report_absent","account_readiness_absent","authorization_root_absent","private_root_absent"]` with no extra, missing, duplicate or reordered item.
...
micromamba run -n qi-p0 python .agent/harness/scripts/live_cycle_authorization.py prepare-resume \
  ...
  --safe-override /var/tmp/quality-inspection-gdt10e-20260802-db2265ae5e7d/safe.env \
  --readiness /var/tmp/quality-inspection-gdt10e-20260802-db2265ae5e7d/account-readiness.json \
  --runtime-acceptance "/home/reggie/vscode_folder/Quality_Inspection/.worktrees/gdt10e-retry-archive-continuation/.agent/harness/runs/${GDT10E_RUN_ID:?}/reports/provider-account-runtime-acceptance.json" \
  --run-id "${GDT10E_RUN_ID:?}"
QI_LIVE_CYCLE_AUTHORIZATION_REF=/var/tmp/quality-inspection-gdt10e-20260802-db2265ae5e7d/authorization \
QI_LIVE_CYCLE_OVERRIDE_REF=/var/tmp/quality-inspection-gdt10e-20260802-db2265ae5e7d/live.env \
