...
Use a high-entropy marker as fake API key and a distinct fake workspace。Assert exact bundle passes；key/workspace/model/cycle mutation fails；stdout/stderr/log/error/repr/public evidence do not contain marker、workspace、salt或private binding。
...
Expected：all pass；architecture gate forbids private salt/binding fields in run/live/receipt schemas and logs。
