...
| P0 Contract ID | Global Contract ID | Implementation Only | Related Global Contract IDs | Stable P0 Requirement | Owner | Task ID | Tier | Verification Selector | Blocking Level | Current Status | Implementation Reason |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `P0-RES-001` | `REV-002` | `false` | `[SYS-006, CAND-005]` | Processing 可先绑定同一输入 identity 的 immutable preview revisions，terminal 时精确一次冻结带 versioned `complete` or `partial_review_required` completeness 的 immutable `raw_automatic_result`；working edits 或 late Provider response 不得原地修改 | Automatic-result repository | `D3-T2` | integration | `pytest backend/tests/integration/test_result_layers.py backend/tests/integration/test_recognition_preview.py backend/tests/integration/test_task_idempotency.py -q` | `blocking` | `not_run` | 既有 raw-result immutability proof 仍有效，但 versioned completeness、preview binding、late-response 和 exact-once delta 尚无执行证据。 |
