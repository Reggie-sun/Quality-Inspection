...
| `P0-REC-005` | `null` | `true` | `[SYS-003, CAND-004, PROV-001, PROV-002, PROV-003]` | `qwen3-vl-plus` 只复核局部 candidate 或 suspicious visual-observation crop，输出必须通过各自冻结 JSON Schema；Provider 不拥有正式 disposition | Vision Advisor adapter | `D7-T2` | provider-contract | `pytest backend/tests/contract/test_qwen_vl_provider.py backend/tests/contract/test_qwen_symbol_provider.py -q` | `blocking` | `not_run` | 既有 passed text-candidate Provider proof 仍有效，但不能满足新增 visual-symbol schema、local validator 与 input-registration delta；`not_run` 是 Harness 对 pending visual proof 的既有状态。 |
...
| `P0-REC-009` | `CAND-005` | `false` | `[PDF-007, CAND-002]` | Coverage Ledger 中每个疑似工程 text/visual observation 恰有一个 primary disposition、source 与 coordinates；未调度或预算溢出的 visual observation 为 blocking | Coverage Owner | `D7-T2` | unit | `pytest backend/tests/unit/candidates/test_coverage.py::test_every_suspicious_observation_has_complete_disposition backend/tests/unit/candidates/test_coverage.py::test_visual_candidate_has_one_complete_coverage_entry -q` | `blocking` | `not_run` | 既有 passed text coverage proof 仍有效，但不能满足新增 visual observation identity、scheduling 与 completeness delta；`not_run` 是 Harness 对 pending visual proof 的既有状态。 |
| `P0-REC-010` | `CAND-005` | `false` | `[CAND-006]` | `ambiguous` text/visual source 可进入审核并保持可定位、promote/ignore；缺 disposition/source/coordinates 或冲突归属为 blocking | Coverage Veto Gate | `D7-T2` | unit | `pytest backend/tests/unit/candidates/test_coverage.py::test_ambiguous_is_reviewable_but_incomplete_is_blocking backend/tests/unit/candidates/test_coverage.py::test_visual_reference_noninspection_and_ambiguous_are_distinct -q` | `blocking` | `not_run` | 既有 passed text ambiguity proof 仍有效，但不能满足新增 visual source-review 与 reference/non-inspection distinction delta；`not_run` 是 Harness 对 pending visual proof 的既有状态。 |
