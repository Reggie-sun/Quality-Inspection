...
- 已完成行的 `passed` 只来自 sealed task/full-P0 results。D6-T3 与 D7-T1 已由各自 task receipts 投影；D7-T2 的 `P0-BAL-006/007/014`、`P0-UI-004` 和 `P0-ACC-001..006` 由 sealed full-P0 run `20260723T025857305843Z-609c61d3` 的 111 个 passed contract results 投影。该 run 的 formal overall verdict 仍由 D7-T3 policy gate 阻断，不能把 row projection 解释为 D7-T3 已完成。
...
- requirement/task/selector变化会改变 `contract_definition_hash`；D7-T2 已用最终 executable/definition identity 生成 fresh current-four live evidence，D7-T3 只可在其计划边界内执行 final receipt/rollback policy closure。
