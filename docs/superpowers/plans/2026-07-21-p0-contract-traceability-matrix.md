...
| `P0-REC-005` | `null` | `true` | `[SYS-003, CAND-004, PROV-001, PROV-002, PROV-003]` | Proposal admission 与 reason-coded escalation 独立；native/OCR/deterministic geometry 已明确解析的 observation 不调用 Provider，只有 unresolved/conflicting/unknown/unparseable 局部 ROI 可由 `qwen3-vl-plus` 复核并通过冻结 JSON Schema，Provider 不拥有正式 disposition | Candidate Advisor boundary | `D7-T2` | provider-contract | `pytest backend/tests/unit/candidates/test_local_symbol_resolution.py backend/tests/unit/candidates/test_symbol_routing.py backend/tests/unit/candidates/test_advisor.py backend/tests/contract/test_qwen_symbol_provider.py -q` | `blocking` | `not_run` | 既有 Provider/schema proof 仍有效，但 production uncertainty router 的 local-skip、reason-code、zero-call 与 Provider non-owner delta 尚无执行证据。 |
...
| `P0-REC-009` | `CAND-005` | `false` | `[PDF-007, CAND-002, REV-002]` | Coverage Ledger 中每个 admitted text/visual observation 恰有一个 primary disposition、source 与 coordinates；预算拒绝或局部 Provider 失败保留 unresolved reason 和完整 coverage，并使 terminal completeness 为 `partial_review_required`，不得静默丢弃 | Coverage Owner | `D7-T2` | unit | `pytest backend/tests/unit/candidates/test_symbol_routing.py::test_every_admitted_observation_has_exactly_one_disposition backend/tests/integration/test_symbol_recognition_pipeline.py backend/tests/integration/test_result_layers.py -q` | `blocking` | `not_run` | 既有 text/visual coverage proof 仍有效，但 production route 的 exact-one routing、budget/partial retention 和 completeness delta 尚无执行证据。 |
| `P0-REC-010` | `CAND-005` | `false` | `[CAND-006, REV-002]` | `ambiguous`/unresolved text/visual source 可进入审核并保持可定位、promote/ignore；缺 identity/disposition/source/coordinates、冲突归属或证据损坏仍为 blocking，Quality Owner 必须关闭 unresolved evidence 后才可越过正式 gate | Coverage Veto Gate | `D7-T2` | unit | `pytest backend/tests/unit/candidates/test_local_symbol_resolution.py::test_conflicting_family_evidence_escalates backend/tests/unit/candidates/test_symbol_routing.py::test_unknown_reason_fails_closed backend/tests/integration/test_review_freeze.py backend/tests/integration/test_balloon_validation.py -q` | `blocking` | `not_run` | 既有 ambiguity/source-review proof 仍有效，但 fail-closed corruption、partial unresolved closure 和 Quality Owner veto delta 尚无执行证据。 |
...
| `P0-UI-004` | `REV-003` | `false` | `[REV-002, DIAG-003]` | Product Design 工作台同页区分 candidate/source、全部 active required balloons 与 leader；production route 处理中只读显示 backend-owned `local_ready -> vlm_enriching -> terminal` preview/completeness/call-budget summary，frontend 不推导业务成功或伪造 working copy | Workbench presentation executor (non-owner projection) | `D7-T2` | frontend | `npm --prefix frontend test -- --run src/components/workbench/RecognitionPreviewApp.test.tsx src/components/workbench/RecognitionSummary.test.tsx src/components/workbench/InspectionItemTable.test.tsx src/components/balloons/BalloonOverlay.test.tsx` | `blocking` | `not_run` | 既有 workbench statistics/overlay proof 仍有效，但 backend-owned progressive preview、refresh 和 frontend non-owner delta 尚无执行证据。 |
...
| `P0-UI-006` | `REV-004` | `false` | `[REV-002, DIAG-003]` | 核心审核表单支持 keep/exclude/edit/add/split/confirmation/balloon-required；progressive preview 在真实 working copy 创建前只读且 mutation controls disabled，frontend 不把 preview 提交为正式 review 语义 | Review-command Owner; frontend executor | `D4-T3` | frontend | `npm --prefix frontend test -- --run src/components/workbench/RecognitionPreviewApp.test.tsx src/components/review/ReviewPanel.test.tsx src/components/workbench/InspectionWorkbench.test.tsx` | `blocking` | `not_run` | 既有 review-command UI proof 仍有效，但 preview read-only/mutation-disable 和 frontend non-owner delta 尚无执行证据。 |
...
| `P0-UI-008` | `REV-006` | `false` | `[REV-002, CAND-005]` | 明确 Confirm 动作只可在真实 working copy、complete coverage 与 Quality Owner closure 后冻结 reviewed result；`partial_review_required` preview/result、普通 Save 或 frontend terminal label 均不会 freeze、balloon 或 export | Review freeze Owner; frontend executor | `D5-T3` | frontend | `pytest backend/tests/integration/test_review_freeze.py backend/tests/integration/test_balloon_validation.py -q && npm --prefix frontend test -- --run src/components/workbench/RecognitionPreviewApp.test.tsx src/components/workbench/FreezeReviewButton.test.tsx` | `blocking` | `not_run` | 既有 Save/Confirm distinction proof 仍有效，但 partial completeness/Quality Owner veto 与 preview UI delta 尚无执行证据。 |
...
| P0 Contract ID | Global Contract ID | Implementation Only | Related Global Contract IDs | Stable P0 Requirement | Owner | Task ID | Tier | Verification Selector | Blocking Level | Current Status | Implementation Reason |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `P0-RES-001` | `REV-002` | `false` | `[SYS-006, PRJ-002, CAND-005]` | Result reader 必须先按登记默认值兼容读取新增字段；processing 绑定 project intake 时冻结的 `recognition_mode`、`recognition_router_version` 与同一 input identity，可先关联 immutable preview revisions，terminal 时精确一次冻结带 versioned `complete` or `partial_review_required` completeness 的 immutable `raw_automatic_result`；runtime setting、working edits 或 late Provider response 不得原地改变结果语义 | Automatic-result repository | `D3-T2` | integration | `pytest backend/tests/integration/test_result_layers.py backend/tests/integration/test_project_intake_api.py backend/tests/integration/test_project_status_api.py backend/tests/integration/test_recognition_preview.py backend/tests/integration/test_task_idempotency.py -q` | `blocking` | `not_run` | 既有 raw-result immutability proof 仍有效，但 reader-first result、project-frozen mode/router identity、versioned completeness、preview binding、late-response 和 exact-once delta 尚无执行证据。 |
...
| `P0-RES-008` | `PROV-005` | `false` | `[DIAG-003]` | 分开保存 visual primary groups `4/page, 8/project`、text+visual actual attempts `16/page`、retry、`45s/page, 90s/project` wall budget、duration/cost/cache provenance；retry 不增加 primary 但消耗 actual/wall。`cross_project_cache_status=blocked_missing_security_scope_owner`；`project_local_content_cache_allowed=true` | Provider-call policy Owner | `D2-T2` | provider-contract | `pytest backend/tests/unit/candidates/test_symbol_routing.py::test_planner_enforces_page_project_and_unified_call_budgets backend/tests/unit/candidates/test_symbol_routing.py::test_retry_preserves_primary_count_but_consumes_actual_and_wall_budget backend/tests/unit/candidates/test_symbol_cache.py backend/tests/contract/test_provider_call_records.py backend/tests/contract/harness/test_symbol_eval_contract.py -q` | `blocking` | `not_run` | 既有 minimum call statistics proof 仍有效，但独立 primary/actual/wall accounting、compatible cache provenance 和 offline evidence delta 尚无执行证据；跨项目 cache 明确 blocked，不得报告 passed。 |
...
| `P0-ACC-002` | `null` | `true` | `[CAND-004, CAND-005, REV-002, DIAG-003]` | Production uncertainty route 对四份 frozen input 保留完整 source/coordinates/disposition/unresolved evidence；Quality Owner 逐页确认应检验项均 keep/add、excluded 有明确 evidence，且 recall 不低于已登记 gate。空结果、coverage blocking、mass exclusion、`partial_review_required` 或低 call count 均不得冒充 completeness success | Quality Owner candidate/coverage gate | `D7-T2` | live-e2e | `phase://live/candidates?input_set=current-four&recognition_mode=production_uncertainty` | `blocking` | `not_run` | 既有 current-four legacy/high-recall acceptance 仍有效；production uncertainty route 的 sealed comparison、recall regression gate、partial evidence 和 Quality Owner verdict 尚未执行，Harness 只编排证据而非业务 Owner。 |
...
| `P0-ACC-007` | `PRJ-005` | `false` | `[CAND-005, REV-002, EXP-006]` | 局部 ROI Provider/budget 故障保留 sibling 正式证据并只形成 `partial_review_required`；identity/conflict/schema-provenance corruption、storage、template、font 或任一正式子产物故障仍阻止 result/formal success/download。Provider、frontend 和 Harness 均不能降级此 Veto Gate | Processing / Review / Export formal-success Veto Gate | `D7-T1` | failure-e2e | `phase://failure/no-silent-success?recognition_mode=production_uncertainty` | `fatal` | `not_run` | 既有 no-silent-success proof 仍有效，但 localized partial retention、systemic fail-closed 和 production-route Harness evidence 尚未执行。 |
