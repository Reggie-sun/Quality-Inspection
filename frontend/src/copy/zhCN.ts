...
export const zhCN = {
  ...
  drawingList: {
    ...
    moreActions: (fileName: string) => `打开 ${fileName} 的更多操作`,
    reprocess: "重新识别",
    delete: "删除图纸",
    reprocessTitle: (fileName: string) => `重新识别 ${fileName}？`,
  ...
  inspection: {
    ...
    resolveSipException: "解决并保存 SIP 异常",
    exceptionFieldsPrompt: "只需补充以下 SIP 异常字段",
    exceptionFieldRequired: "需补充",
    editOtherSipFields: "查看或修改其他 SIP 字段",
    ...
    gdtModifiers: {
      maximum_material_condition: "最大实体要求",
      least_material_condition: "最小实体要求",
      regardless_of_feature_size: "独立原则",
      unknown: "未知修饰符",
    },
    gdtDatum: (datum: string) => `基准 ${datum}`,
    gdtEditor: {
      title: "结构化几何公差",
      subtype: "公差类型",
      frame: (index: number) => `控制框第 ${index} 行`,
      toleranceValue: "公差值",
      diameterModifier: "直径符号",
      modifier: "材料条件",
      noModifier: "无",
      datum: (index: number) => `基准 ${index}`,
      save: "保存几何公差",
      invalidValue: "请输入大于 0 的小数",
      invalidDatum: "基准必须是一个大写字母",
      unknownHint: "请补全未确认的几何公差结构。",
    },
    collisions: {
